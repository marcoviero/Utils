...
class Field_catalogs:
	...
	def get_subpop_ids(self, znodes, mnodes, pop_dict, linear_mass=1, lookback_time = False, ndecimal=2):
		self.subpop_ids = {}
		decimal_pref = '{:.'+str(ndecimal)+'f}'
		...
		for iz in range(len(znodes[:-1])):
			zn = znodes[iz:iz+2]
			z_suf = decimal_pref.format(zn[0])+'-'+decimal_pref.format(zn[1])
			for jm in range(len(mnodes[:-1])):
				mn = mnodes[jm:jm+2]
				m_suf = decimal_pref.format(mn[0])+'-'+decimal_pref.format(mn[1])
				for k in pop_dict:
					if linear_mass == 1:
					...
					#pdb.set_trace()
					try:
						arg = clean_args('z_'+z_suf+'__m_'+m_suf+'_'+pop_dict[k])
					except:
						arg = clean_args('z_'+z_suf+'__m_'+m_suf+'_'+k)
					#self.subpop_ids['z_'+clean_args(str(decimal_pref(znodes[iz])))+'_'+clean_args(str(decimal_pref(znodes[iz+1])))+'__m_'+clean_args(str(decimal_pref(mnodes[jm])))+'_'+clean_args(str(decimal_pref(mnodes[jm+1])))+'_'+pop_dict[k]] = self.table.ID[ind_mz].values
					self.subpop_ids[arg] = self.table.ID[ind_mz].values
